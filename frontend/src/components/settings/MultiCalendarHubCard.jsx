      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Kết nối iCal Secret URL hoặc Google/Outlook OAuth2 để phát hiện xung đột lịch</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            disabled={syncing}
            className="flex-1 sm:flex-initial px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-md shadow-indigo-600/20 disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${syncing ? 'animate-spin' : ''}`} />
            <span>{syncing ? 'Đang Quét...' : 'Đồng Bộ & Quét Trùng'}</span>
          </button>

          <button
            onClick={onOpenAddModal}
            className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-md shadow-emerald-600/20 shrink-0"
          >
            <Plus className="w-3.5 h-3.5" /> <span>Thêm Email</span>
          </button>
        </div>
      </div>
          ...
          calendars.map((cal) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: cal.colorTag || cal.color_tag || '#3b82f6' }} />
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-semibold text-xs text-white truncate">{cal.accountName || cal.account_name}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono">
                      {cal.calendarType || cal.calendar_type}
                    </span>
                  </div>
                  <span className="text-[11px] text-slate-400 font-mono truncate block">{cal.emailAddress || cal.email_address}</span>
                </div>
              </div>
