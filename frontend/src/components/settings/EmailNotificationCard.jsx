      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="pt-2 flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-2 shadow-md shadow-indigo-600/20 disabled:opacity-50"
          >
          ...
          {emailTestResult && (
            <div className={`text-xs flex items-center gap-1.5 font-semibold ${emailTestResult.sent ? 'text-emerald-400' : 'text-rose-400'}`}>
              {emailTestResult.sent ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
              {emailTestResult.sent ? `Đã gửi thử tới ${emailTestResult.recipient}!` : 'Chưa gửi được (Kiểm tra SMTP)'}
            </div>
          )}
        </div>
      </div>
